...
\hypersetup{
  ...
  pdfauthor={Y.M.A.P. Rajapaksha ~Department of Statistics, Faculty of Applied Sciences ~University of Sri Jayewardenepura, Sri Lanka},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\author{Y.M.A.P. Rajapaksha ~Department of Statistics, Faculty of
Applied Sciences ~University of Sri Jayewardenepura, Sri Lanka}
\date{}
\begin{document}
\maketitle
...
dashboards is presented in Table \ref{tab:dashboard-summary}.
...
Table \ref{tab:dashboard-features} compares the design characteristics
and visualization techniques used in the six reviewed dengue dashboards.
The dashboard numbers correspond to those listed in Table
\ref{tab:dashboard-summary}. The comparison focuses on the number of
tabs, background theme, suitability for a single-screen layout, data
...
weekly epidemiological reports published by the Epidemiology Unit,
Ministry of Health, Sri Lanka, and the Weekly Dengue Updates published
by the National Dengue Control Unit, Ministry of Health, Sri Lanka.
These data were extracted through web scraping using the
\texttt{convert\_slwer\_to\_tidy()} function available in the
...
Surveillance Dashboard}. The climate data used in this study were
downloaded from the
...
To get an overall idea of the methodologies, see Table
\ref{tab:methods}.
...
tab contains interactive visualizations and analytical outputs. Table
\ref{tab:tabs} provides a description of each tab and summarizes its key
content.
